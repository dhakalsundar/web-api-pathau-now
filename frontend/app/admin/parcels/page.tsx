...
export default function AdminShipmentsPage() {
  ...
  const fetchShipments = async () => {
    try {
      ...
      if (filters.search) {
        response = await adminService.searchAdminParcels(filters.search, page, pageSize);
      } else {
        response = await adminService.getAllAdminParcels(page, pageSize, filterObj);
      }
  ...
  const handleUpdateStatus = async (shipmentId: string, newStatus: string) => {
    try {
      ...
      await adminService.updateAdminParcelStatus(shipmentId, newStatus);
      setSuccess(`Parcel status updated to ${newStatus}`);
      fetchShipments();
      setExpandedId(null);
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-4xl font-bold text-gray-900"> Parcels</h1>
                  ...
                  >
                    ...
                    >
                      <div className="grid grid-cols-7 items-center gap-4">
                        ...
                        {/* Price */}
                        <div>
                          <p className="text-sm font-bold text-gray-900">Rs{shipment.price}</p>
                        </div>

                        {/* Rider */}
                        <div>
                          <p className="text-sm text-gray-600">{shipment.riderId ? ' Assigned' : ' Unassigned'}</p>
                        </div>
                        ...
                        <div className="grid md:grid-cols-3 gap-8">
                          ...
                          <div>
                            ...
                            <div className="space-y-4">
                              ...
                              {/* View Button */}
                              <button
                                onClick={() => router.push(`/admin/parcels/${shipment.trackingNumber}`)}
                              ...
                              {/* Edit Button */}
                              <button
                                onClick={() => router.push(`/admin/parcels/${shipment.trackingNumber}/edit`)}
