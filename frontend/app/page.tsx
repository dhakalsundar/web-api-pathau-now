        ...
        <div className="container mx-auto px-6">
          ...
          <div className="border-t border-gray-700 pt-8 text-center text-sm">
            <p>&copy; 2024 PathauNow. All rights reserved.</p>
              ...
              {/* Using existing TrackBox component for consistency */}
              <TrackBox />
            </div>
            ...
            <div className="mt-8 grid grid-cols-2 sm:grid-cols-3 gap-4">
              <div className="bg-white p-4 rounded-lg shadow-sm">
                ...
                <div className="text-sm font-semibold mt-2">Real-time status</div>
                <div className="text-xs text-slate-500">Clear updates from pickup to delivery.</div>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="text-2xl">🚚</div>
                <div className="text-sm font-semibold mt-2">Rider friendly</div>
                <div className="text-xs text-slate-500">Assign and monitor riders quickly.</div>
              </div>
              <div className="bg-white p-4 rounded-lg shadow-sm">
                <div className="text-2xl">🔎</div>
                <div className="text-sm font-semibold mt-2">Powerful search</div>
                <div className="text-xs text-slate-500">Search by tracking ID, sender, or recipient.</div>
              </div>
            </div>

            <div className="mt-8 text-sm text-slate-500">Trusted by small businesses and couriers. Demo tracking numbers: <span className="font-mono">PNDEMO1</span>, <span className="font-mono">PNDEMO2</span></div>
          </div>
        </div>

        <div className="lg:col-span-5">
          <div className="rounded-xl overflow-hidden shadow-lg">
            <img alt="Delivery scooter" src="https://images.unsplash.com/photo-1520975698511-3f1c1d8a1a3b?auto=format&fit=crop&w=1200&q=80" className="w-full h-80 object-cover" />
          </div>

          <div className="mt-6 grid grid-cols-1 gap-4">
            <div className="bg-white p-4 rounded-lg shadow-sm">
              <div className="text-sm font-semibold">Convenient booking</div>
              <div className="text-xs text-slate-500">Schedule pickups and print labels.</div>
            </div>
            <div className="bg-white p-4 rounded-lg shadow-sm">
              <div className="text-sm font-semibold">Admin dashboard</div>
              <div className="text-xs text-slate-500">Track volumes, revenue and riders.</div>
            </div>
          </div>
        </div>
      </section>

      <footer className="siteFooter mt-12">
        <div className="container mx-auto px-6">
          <div className="footerGrid">
            <div>
              <h4 className="font-semibold">PathauNow</h4>
              <div className="text-xs text-slate-500 mt-2">Parcel tracking simplified.</div>
            </div>
            <div className="text-right">
              <div className="text-xs text-slate-500">© {new Date().getFullYear()} PathauNow</div>
            </div>
          </div>
        </div>
      </footer>
